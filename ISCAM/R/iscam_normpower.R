...
iscam_normpower <- function(LOS, n, prob1, alternative, prob2 = NULL, explain = FALSE){
  ...
  mean = prob1
  std <- round(sqrt(prob1*(1-prob1)/n), digits = 4)
  thisx = 0:n
  maintitle <-substitute(paste("Normal (", mu == x1, ", ", sigma == x2, ")",),
                         list(x1 = mean, x2 = std))
  ...
  if (alternative=="less") {
    ...
    subtitle <- paste("P(p-hat \u2264 ", rr, ") = ", showprob1, sep = "") #creating subtitle
    ...
    rr <- round(qnorm(LOS, mean, std, FALSE), digits = 4)
    this.prob1=1-pnorm(rr, mean, std)
    showprob1=round(this.prob1, digits=4)
    subtitle <- paste("P(p-hat \u2265 ", rr, ") = ", showprob1, sep = "")
    df <- data.frame(x = thisx, y = dnorm(thisx, mean, std))
    plot <- ggplot(df, aes(x = x, y = y, width = 0.15))
    if (!isTRUE(explain)){
      plot1 <- plot +
    ...
    lowerrr= round(qnorm(LOS/2, mean, std), 4)
    upperrr= round(qnorm(LOS/2, mean, std, FALSE), 4)
    ...
    subtitle <- paste("P(p-hat \u2264 ",  lowerrr, ") + P(p-hat \u2265 ", upperrr, ") = ", showprob1, sep="")
    df <- data.frame(x = thisx, y = dnorm(thisx, mean, std))
    plot <- ggplot(df, aes(x = x, y = y, width = 0.15))
    if (!isTRUE(explain)){
      plot1 <- plot + 
  ...
  if (!is.null(prob2)){
    ...
    maintitle2 <- substitute(paste("Normal (", mu==x1,", ", sigma==x2, ")", ), list(x1=mean2, x2=std2))
    if (alternative=="less") {
      rr = round(qnorm(LOS, mean, std), 4)
      this.prob2=pnorm(rr, mean2, std2)
      showprob2=round(this.prob2, digits=4)
      subtitle2 <- paste("P(p-hat \u2264 ", rr, ") = ", showprob2, sep = "")
      ...
      showlowerprob2=round(lowerprob2, digits=4); showupperprob2=round(upperprob2, digits=4)
      showprob2=round(lowerprob2+upperprob2, digits=4)
      showupperrr = round(upperrr, digits=4); showlowerrr=round(lowerrr, digits=4)
      subtitle2 <- paste("P(p-hat \u2264",  showlowerrr, ")+P(p-hat \u2265",showupperrr, ") =", showlowerprob2, "+", showupperprob2, "\n =", showprob2)
      df <- data.frame(x = thisx, y = dnorm(thisx, mean2, std2))
      plot <- ggplot(df, aes(x = x, y = y, width = 0.15))
      if (!isTRUE(explain)){
        plot2 <- plot + 
    ...
    plot1 <- plot1 + xlim(minx, maxx) +
      ...
      geom_segment(aes(x=minx,xend=maxx,y=0,yend=0), color = "dodgerblue")
