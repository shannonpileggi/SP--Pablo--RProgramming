...
iscam_normprob <- function(xval, mean = 0, sd = 1, direction, label = NULL, xval2=NULL, digits = 4){
  ...
  if (direction=="below"){
    ...
    mySubtitle <- paste("Pr(X \u2264 ", xvallabel, ") = ", showprob, sep = "")
    ...
    mySubtitle <- paste("Pr(", xvallabel, " \u2264 X \u2264 ", xval2label, ") = ", showprob, sep = "")
    plot1 <- ggplot(data, aes(x = x, y = y, width = 0.25)) + 
      stat_function(fun = dnorm,  #drawing t density curve
                    args = list(mean = mean, sd = sd),
                    color = "dodgerblue") +
      stat_function(fun = dnorm,  #shading in t curve
                    args = list(mean = mean, sd = sd),
                    xlim = c(xval, xval2),
                    geom = "area", 
                    color = "dodgerblue4",
    ...
    showprob <- format(normprob, digits = digits)
    mySubtitle <- paste("Pr(X ", "\u2264 ", xvallabel, ") and Pr(X ",  "\u2265 ", xval2label, ") = ", showprob, sep = "")
    plot1 <- ggplot(data, aes(x = x, y = y, width = 0.25)) + 
      stat_function(fun = dnorm,  #drawing normal density curve
                    args = list(mean = mean, sd = sd),
                    color = "dodgerblue") +
  ...
  finalplot <- plot1 + geom_segment(aes(x = minx, xend = maxx, y = 0, yend = 0), color = "dodgerblue")
  print(finalplot)
